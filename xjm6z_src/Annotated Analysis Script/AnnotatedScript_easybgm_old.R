...
library(remotes)
#install_github("KarolineHuth/easybgm", upgrade = "never")
#install_github("MaartenMarsman/bgms")
library(easybgm)
library(BDgraph)
library(BGGM)
library(qgraph)
...
bdgraph_res <- easybgm::easybgm(data = data, type = "mixed", 
                           package = "BDgraph", not.cont = c(rep(0, 3), rep(1, 10)), 
...
bggm_res <- easybgm::easybgm(data = data, type = "mixed", 
                       package = "BGGM", not.cont = c(rep(0, 3), rep(1, 10)), 
                       centrality = TRUE, save = TRUE, iter = Nsamples)
...
plot_posteriorstructure(res, as.BF = F) #(NOTE: not usable with BGGM)

# 0.b. Posterior complexity probability
plot_posteriorcomplexity(res) #(NOTE: not usable with BGGM)
